const filterOp = {
	...
	arrElemMatch: { mongo: "$elemMatch", sql: null, txt: "Array Elements Match" },
...
class Filter<ColNameT> implements IFilter {
	...
	toSql(paramName: string, params: Record<string, object | number | string>): IFilterSqlResult {
		if (!paramName) throw new Error("Param name is required.");

		const fieldName = this.columnName as string;
		const sqlOp = filterOp[this.op].sql;
		if (!sqlOp) throw new Error("This filter is not supported in SQL");
		const cmd = `${fieldName} ${sqlOp} ${paramName}`;
		if (params[paramName]) throw new Error(`Param Name ${paramName} is already assigned`);
	...
	public toMongoQuery(): object {
		const result: Record<string, Record<string, object | number | string>> = {};
		const fieldName = this.columnName as string;
